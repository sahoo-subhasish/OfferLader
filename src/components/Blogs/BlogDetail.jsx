  ...
  return (
    ...
    <section className="flex flex-col animate-in fade-in duration-500 max-w-[1200px] mx-auto p-4 md:p-6 mb-20 md:pt-12 w-full h-full text-white" data-color-mode="dark">
      ...
      <div className="flex items-center justify-between border-y border-[#262626] py-5 mb-16 px-2 sm:px-4">
        <button 
          onClick={handleLike}
          className={`group flex items-center gap-2 sm:gap-3 px-3 sm:px-5 py-2.5 rounded-full transition-all font-semibold border ${
            hasLiked 
        ...
        >
          ...
          <span className="text-sm">{blog.likes?.length || 0}<span className="hidden sm:inline"> Likes</span></span>
        </button>

        <div className="flex items-center gap-0 sm:gap-3">
          {/* Share Button */}
          <button
            id="share-blog-detail"
            onClick={() => handleShare({ title: blog.title, url: `/blogs/${id}` })}
            className="group flex items-center gap-2 px-3 sm:px-4 py-2.5 rounded-full border border-transparent hover:border-[#333] hover:bg-[#1a1a1a] text-neutral-400 hover:text-white transition-all font-semibold text-sm"
          ...
          {/* Comments count */}
          <div className="flex items-center gap-2 px-3 sm:px-4 py-2.5 text-neutral-400 font-medium text-sm">
            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 11.5a8.38 8.38 0 0 1-.9 3.8 8.5 8.5 0 0 1-7.6 4.7 8.38 8.38 0 0 1-3.8-.9L3 21l1.9-5.7a8.38 8.38 0 0 1-.9-3.8 8.5 8.5 0 0 1 4.7-7.6 8.38 8.38 0 0 1 3.8-.9h.5a8.48 8.48 0 0 1 8 8v.5z"></path></svg>
            <span>{comments.length}<span className="hidden sm:inline"> Comments</span></span>
          </div>
        </div>
      </div>
      ...
      {/* Copied toast */}
      {copied && (
        <div className="fixed bottom-6 right-6 z-50 flex items-center gap-2.5 bg-[#48D2A0] text-black text-xs font-bold px-4 py-3 rounded-xl shadow-lg animate-in fade-in slide-in-from-bottom-3">
